...
SEMANTIC_FUSION_DETACH_RELIABILITY_INPUTS="${SEMANTIC_FUSION_DETACH_RELIABILITY_INPUTS:-}"
...
COMMON_OPTS=(
  ...
  train.protocol_id "${PROTOCOL_ID:-legacy}"
  train.validation_greedy "$(bool_word "${VALIDATION_GREEDY:-0}")"
  data.isolate_loader_rng "$(bool_word "${ISOLATE_LOADER_RNG:-0}")"
  data.seed_workers "$(bool_word "${SEED_WORKERS:-0}")"
...
if [ -n "$NUM_WORKERS" ]; then
  COMMON_OPTS+=(data.num_workers "$NUM_WORKERS")
fi
...
if [ -n "$SEMANTIC_FUSION_DETACH_RELIABILITY_INPUTS" ]; then
  COMMON_OPTS+=(model.semantic_fusion_detach_reliability_inputs "$(bool_word "$SEMANTIC_FUSION_DETACH_RELIABILITY_INPUTS")")
fi

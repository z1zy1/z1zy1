...
ONLY_EXP=""
DRY_RUN=0
FAIL_LOG="$EXP_ROOT/7_4_followup_test_failures.log"
...
usage() {
  echo "Usage: bash scripts/run_7_4_followup_test_and_summary.sh [--only_exp EXP] [--force] [--dry_run]" >&2
}
...
strict_selection_note() {
  local best_json="$1"
  local met source step
  met="$(json_value "$best_json" strict_constraints_met)"
  source="$(json_value "$best_json" selected_source_exp_name)"
  step="$(json_value "$best_json" selected_epoch_or_step)"
  if [ "$met" = "True" ] || [ "$met" = "true" ]; then
    echo "strict validation constraints met; selected_source=$source step=$step"
    return 0
  fi
  local bleu_gap cider_gap spice_gap
  bleu_gap="$(json_value "$best_json" threshold_shortfall.Bleu_4)"
  cider_gap="$(json_value "$best_json" threshold_shortfall.CIDEr)"
  spice_gap="$(json_value "$best_json" threshold_shortfall.SPICE)"
  echo "strict validation constraints not all met; selected nearest checkpoint; selected_source=$source step=$step; shortfall Bleu_4=$bleu_gap CIDEr=$cider_gap SPICE=$spice_gap"
}

grid_selection_note() {
  local best_json="$1"
  local source step strategy
  source="$(json_value "$best_json" selected_source_exp_name)"
  step="$(json_value "$best_json" selected_epoch_or_step)"
  strategy="$(json_value "$best_json" selection_strategy)"
  echo "cross-experiment selection; selected_source=$source step=$step strategy=$strategy"
}
...
configure_exp() {
  ...
  case "$exp" in
    levir_mci_ultrashort_caption_ft_100_lr005_dense10|levir_mci_ultrashort_caption_ft_80_lr004_dense10|levir_mci_ultrashort_caption_ft_80_lr003_dense10|levir_mci_ultrashort_grid_reselect_strict_nearest)
    ...
    second_cc_crossattn_pd09_lsem0000)
      export DATASET=second_cc DATA_ROOT="$SECOND_ROOT" ANNO="$SECOND_ROOT/second_cc_aug_captions_reformat.json" CHANGEFLAG_JSON="$SECOND_ROOT/SECOND-CC-AUG.json" EVAL_CHANGE_NOCHANGE_SPLIT=1
      export BASE_CFG="configs/dynamic/transformer_second_cc_aug_sgc_card.yaml" MODEL_TYPE=sgc_card USE_CHANGE_MASK=0 MASK_TYPE=binary ENABLE_AUX_MASK=0 USE_AUX_SEMANTIC=0 USE_SEMANTIC_MAPS=1 SEMANTIC_INPUT_MODE=cross_attention NUM_SEMANTIC_CLASSES=7 USE_SEMANTIC_PARTIAL_DETACH=1 SEMANTIC_DETACH_RATIO=0.9 LMASK=0.0 LSEM=0.0 ;;
    second_cc_crossattn_lsem0000_pdgrid_reselect_balanced)
      export DATASET=second_cc DATA_ROOT="$SECOND_ROOT" ANNO="$SECOND_ROOT/second_cc_aug_captions_reformat.json" CHANGEFLAG_JSON="$SECOND_ROOT/SECOND-CC-AUG.json" EVAL_CHANGE_NOCHANGE_SPLIT=1
      export BASE_CFG="configs/dynamic/transformer_second_cc_aug_sgc_card.yaml" MODEL_TYPE=sgc_card USE_CHANGE_MASK=0 MASK_TYPE=binary ENABLE_AUX_MASK=0 USE_AUX_SEMANTIC=0 USE_SEMANTIC_MAPS=1 SEMANTIC_INPUT_MODE=cross_attention NUM_SEMANTIC_CLASSES=7 USE_SEMANTIC_PARTIAL_DETACH=1 SEMANTIC_DETACH_RATIO=0.8 LMASK=0.0 LSEM=0.0 ;;
    *) echo "Unknown 7.4 follow-up test experiment: $exp" >&2; return 2 ;;
  esac
}

reselect_json_name() {
  case "$1" in
    levir_mci_ultrashort_grid_reselect_strict_nearest) echo best_checkpoint_strict_nearest.json ;;
    second_cc_crossattn_lsem0000_pdgrid_reselect_balanced) echo best_checkpoint_pdgrid_balanced.json ;;
...
select_trained_checkpoint() {
  ...
  case "$exp" in
    levir_mci_ultrashort_caption_ft_100_lr005_dense10|levir_mci_ultrashort_caption_ft_80_lr004_dense10|levir_mci_ultrashort_caption_ft_80_lr003_dense10)
      selector_args+=(
      ...
      ;;
    *) selector_args+=(--strategy spice_constrained_balanced) ;;
  esac
  run_or_log "select best $(basename "$exp_path")" "$PYTHON" scripts/select_best_checkpoint.py "${selector_args[@]}"
}
...
update_done_summary() {
  ...
  if [ -n "$note" ]; then summary_args+=(--notes "$note"); fi
  run_or_log "update summary $exp" "$PYTHON" scripts/update_paper_required_summary.py "${summary_args[@]}"
}
...
run_trained_one() {
  ...
  if [ "$DRY_RUN" -eq 1 ] || [ "$FORCE" -eq 1 ] || [ ! -s "$exp_path/best_checkpoint.json" ]; then
    select_trained_checkpoint "$exp" "$exp_path" || { mark_summary "$exp" failed "Checkpoint selection failed; see $FAIL_LOG."; return 0; }
  fi
  ...
  if [ "$DRY_RUN" -eq 0 ]; then
    case "$exp" in
      levir_mci_ultrashort_caption_ft_100_lr005_dense10|levir_mci_ultrashort_caption_ft_80_lr004_dense10|levir_mci_ultrashort_caption_ft_80_lr003_dense10)
        note="$(strict_selection_note "$exp_path/best_checkpoint.json")" ;;
    esac
  fi
  update_done_summary "$exp" "$exp_path" "$exp_path/best_checkpoint.json" "$note"
}
...
run_reselect_one() {
  ...
  if [ "$DRY_RUN" -eq 1 ] || [ "$FORCE" -eq 1 ] || [ ! -s "$best_json" ]; then
    ...
    run_or_log "reselect $exp" bash scripts/run_7_4_followup_reselect.sh "${reselect_args[@]}" || { mark_summary "$exp" failed "Checkpoint reselect failed; see $FAIL_LOG."; return 0; }
  fi
  local status="done"
  ...
  if [ "$DRY_RUN" -eq 0 ]; then
    case "$exp" in
      levir_mci_ultrashort_grid_reselect_strict_nearest)
        note="$(strict_selection_note "$best_json")" ;;
      second_cc_crossattn_lsem0000_pdgrid_reselect_balanced)
        note="$(grid_selection_note "$best_json")" ;;
    esac
  fi
  update_done_summary "$exp" "$exp_path" "$best_json" "$note"
}
...
if [ "$FAILURES" -gt 0 ]; then
  echo "$FAILURES 7.4 follow-up test/update step(s) failed. See $FAIL_LOG" >&2
  exit 1
fi

echo "7.4 follow-up test and summary flow complete."
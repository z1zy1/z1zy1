...
ONLY_EXP=""
DRY_RUN=0
FAIL_LOG="$EXP_ROOT/7_4_followup_train_failures.log"
...
usage() {
  echo "Usage: bash scripts/run_7_4_followup_train.sh [--only_exp EXP] [--overwrite] [--dry_run]" >&2
}
...
print_effective_train_env() {
  printf 'TRAIN ENV:'
  for key in EXP_NAME DATASET DATA_ROOT BASE_CFG MODEL_TYPE INIT_CHECKPOINT SEED LR FINETUNE_STEPS SAVE_INTERVAL EVAL_INTERVAL SNAPSHOT_INTERVAL USE_CHANGE_MASK ENABLE_AUX_MASK USE_AUX_SEMANTIC USE_SEMANTIC_MAPS SEMANTIC_INPUT_MODE USE_SEMANTIC_PARTIAL_DETACH SEMANTIC_DETACH_RATIO LMASK LSEM USE_FEATURE_REWEIGHT SELECTION_STRATEGY; do
    local value="${!key-}"
...
configure_exp() {
  ...
  case "$exp" in
    levir_mci_ultrashort_caption_ft_100_lr005_dense10)
      ...
      export BASE_CFG="configs/dynamic/transformer_levir_mci_sgc_card.yaml" MODEL_TYPE=sgc_card USE_CHANGE_MASK=0 MASK_TYPE=binary ENABLE_AUX_MASK=0 USE_AUX_SEMANTIC=0 USE_SEMANTIC_MAPS=0 SEMANTIC_INPUT_MODE=none USE_SEMANTIC_PARTIAL_DETACH=0 SEMANTIC_DETACH_RATIO=0.0 LMASK=0.0 LSEM=0.0 LR="${LEVIR_DENSE_LR005:-0.00001}" FINETUNE_STEPS=100 SAVE_INTERVAL=10 EVAL_INTERVAL=10 SNAPSHOT_INTERVAL=10 LOG_INTERVAL=10
      resolve_levir_init_checkpoint ;;
    levir_mci_ultrashort_caption_ft_80_lr004_dense10)
      export DATASET=levir_mci DATA_ROOT="$LEVIR_ROOT" ANNO="$LEVIR_ROOT/levir_mci_captions_reformat.json" CHANGEFLAG_JSON="$LEVIR_ROOT/LevirCCcaptions.json" EVAL_CHANGE_NOCHANGE_SPLIT=1
      export BASE_CFG="configs/dynamic/transformer_levir_mci_sgc_card.yaml" MODEL_TYPE=sgc_card USE_CHANGE_MASK=0 MASK_TYPE=binary ENABLE_AUX_MASK=0 USE_AUX_SEMANTIC=0 USE_SEMANTIC_MAPS=0 SEMANTIC_INPUT_MODE=none USE_SEMANTIC_PARTIAL_DETACH=0 SEMANTIC_DETACH_RATIO=0.0 LMASK=0.0 LSEM=0.0 LR="${LEVIR_DENSE_LR004:-0.000008}" FINETUNE_STEPS=80 SAVE_INTERVAL=10 EVAL_INTERVAL=10 SNAPSHOT_INTERVAL=10 LOG_INTERVAL=10
      resolve_levir_init_checkpoint ;;
    levir_mci_ultrashort_caption_ft_80_lr003_dense10)
      export DATASET=levir_mci DATA_ROOT="$LEVIR_ROOT" ANNO="$LEVIR_ROOT/levir_mci_captions_reformat.json" CHANGEFLAG_JSON="$LEVIR_ROOT/LevirCCcaptions.json" EVAL_CHANGE_NOCHANGE_SPLIT=1
      export BASE_CFG="configs/dynamic/transformer_levir_mci_sgc_card.yaml" MODEL_TYPE=sgc_card USE_CHANGE_MASK=0 MASK_TYPE=binary ENABLE_AUX_MASK=0 USE_AUX_SEMANTIC=0 USE_SEMANTIC_MAPS=0 SEMANTIC_INPUT_MODE=none USE_SEMANTIC_PARTIAL_DETACH=0 SEMANTIC_DETACH_RATIO=0.0 LMASK=0.0 LSEM=0.0 LR="${LEVIR_DENSE_LR003:-0.000006}" FINETUNE_STEPS=80 SAVE_INTERVAL=10 EVAL_INTERVAL=10 SNAPSHOT_INTERVAL=10 LOG_INTERVAL=10
      resolve_levir_init_checkpoint ;;
    second_cc_crossattn_pd07_lsem0000)
    ...
    *) echo "Unknown 7.4 follow-up training experiment: $exp" >&2; return 2 ;;
  esac
}
...
if [ "$FAILURES" -gt 0 ]; then
  echo "$FAILURES 7.4 follow-up training step(s) failed. See $FAIL_LOG" >&2
  exit 1
fi

echo "7.4 follow-up training flow complete."
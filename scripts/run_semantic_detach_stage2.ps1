...
function Run-One {
    ...
    Add-Content -LiteralPath (Join-Path $ResultsRoot 'semantic_detach_stage2_summary.csv') -Value "$Tag,$ExpName,$Seed,$BestCkpt,$SelectDir/best_checkpoint.md,$TestTemplate"
}

function Run-Target {
    param([string]$Target)
    switch ($Target) {
        { $_ -in @('A', 'lmask002_lsem0075') } {
            Run-One 'A' 'lmask002_lsem0075_semantic_detach' '0.02' '0.075' '1111'
            break
        }
        { $_ -in @('B', 'seed1') } {
            Run-One 'B' 'lmask002_lsem01_semantic_detach_seed1' '0.02' '0.10' '1'
            break
        }
        { $_ -in @('C', 'seed2') } {
            Run-One 'C' 'lmask002_lsem01_semantic_detach_seed2' '0.02' '0.10' '2'
            break
        }
        { $_ -in @('D', 'seed3') } {
            Run-One 'D' 'lmask002_lsem01_semantic_detach_seed3' '0.02' '0.10' '3'
            break
        }
        default {
            throw "Unknown stage2 target: $Target. Valid targets: A B C D"
        }
    }
}

function Maybe-SummarizeSeeds {
    $Seed1 = Join-Path $ResultsRoot 'lmask002_lsem01_semantic_detach_seed1/test_results.txt'
    $Seed2 = Join-Path $ResultsRoot 'lmask002_lsem01_semantic_detach_seed2/test_results.txt'
    $Seed3 = Join-Path $ResultsRoot 'lmask002_lsem01_semantic_detach_seed3/test_results.txt'
    if ((Test-Path $Seed1) -and (Test-Path $Seed2) -and (Test-Path $Seed3)) {
        $SummaryCmd = @(
            'python', 'scripts/summarize_semantic_detach_seeds.py',
            '--results_root', $ResultsRoot,
            '--inputs', $Seed1, $Seed2, $Seed3,
            '--output_csv', (Join-Path $ResultsRoot 'lmask002_lsem01_semantic_detach_seed_summary.csv'),
            '--output_md', (Join-Path $ResultsRoot 'lmask002_lsem01_semantic_detach_seed_summary.md')
        )
        Invoke-LoggedCommand -Command $SummaryCmd -LogPath (Join-Path $ResultsRoot 'semantic_detach_seed_summary.log')
    } else {
        Write-Host 'Seed summary skipped because one or more test_results.txt files are missing.'
    }
}

New-Item -ItemType Directory -Force -Path $ResultsRoot | Out-Null
Set-Content -LiteralPath (Join-Path $ResultsRoot 'semantic_detach_stage2_summary.csv') -Value 'tag,exp_name,seed,best_ckpt,selection_md,test_command_template' -Encoding UTF8

foreach ($Target in $Targets) {
    Run-Target $Target
}

Maybe-SummarizeSeeds

Write-Host "Finished stage2 experiments. Summary: $ResultsRoot/semantic_detach_stage2_summary.csv"

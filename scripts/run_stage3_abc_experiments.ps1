...
$DiagRoot = if ($env:DIAG_ROOT) { $env:DIAG_ROOT } else { './diagnostics' }
...
function Run-IndependenceCheck {
    New-Item -ItemType Directory -Force -Path $DiagRoot | Out-Null
    $CheckCmd = @(
        'python', 'scripts/check_experiment_independence.py',
        '--exp1', "$ExpRoot/lmask005_lsem005_semantic_detach",
        '--exp2', "$ExpRoot/lmask005_lsem0075_semantic_detach",
        '--result1', "$ResultsRoot/lmask005_lsem005_semantic_detach",
        '--result2', "$ResultsRoot/lmask005_lsem0075_semantic_detach"
    )
    Invoke-LoggedCommand -Command $CheckCmd -LogPath (Join-Path $DiagRoot 'experiment_independence_report.txt')
}
...
function Write-ArgsFile {
    param(
        ...
        [string]$UseSemanticPartialDetach,
        [string]$SemanticUpdateVisual,
        [string[]]$TrainCommand
    )
    @(
        "tag: $Tag",
        ...
        "use_semantic_partial_detach: $UseSemanticPartialDetach",
        "semantic_update_visual: $SemanticUpdateVisual",
...
python test_card_spot.py `
  ...
  train.use_semantic_partial_detach "$UseSemanticPartialDetach" `
  train.semantic_update_visual "$SemanticUpdateVisual" `
...
function Run-One {
    ...
    if ($UseSemanticDetach -eq 'True') { $TrainCmd += @('--use_semantic_detach') }
    if ($UseSemanticPartialDetach -eq 'True') { $TrainCmd += @('--use_semantic_partial_detach') }
    if ($SemanticUpdateVisual -eq 'True') { $TrainCmd += @('--semantic_update_visual') } else { $TrainCmd += @('--no_semantic_update_visual') }
    $TrainCmd += @(
        'gpu_id', "[$PytorchGpu]",
    ...
    Write-ArgsFile `
        ...
        -UseSemanticPartialDetach $UseSemanticPartialDetach `
        -SemanticUpdateVisual $SemanticUpdateVisual `
    ...
    if (-not $BestCkpt) { throw "Failed to read selected checkpoint from $SelectDir/best_checkpoint.csv" }

    Write-TestTemplate $TestTemplate $ExpName $BestCkpt $LambdaMask $LambdaSemantic $UseSemanticDetach $UseSemanticPartialDetach $SemanticUpdateVisual
    if ($RunTest) {
        $TestCmd = @(
            'python', 'test_card_spot.py',
            '--cfg', $BaseCfg,
    ...
    Add-Content -LiteralPath (Join-Path $ResultsRoot 'stage3_abc_summary.csv') -Value "$Tag,$ExpName,$BestCkpt,$SelectDir/best_checkpoint.md,$TestTemplate"
}

function Run-Target {
    param([string]$Target)
    switch ($Target) {
        { $_ -in @('B', 'lmask002_lsem0075') } {
            Run-One 'B' 'lmask002_lsem0075' '0.02' '0.075' 'False' 'False' 'True'
            break
        }
        { $_ -in @('C', 'partial') } {
            Run-One 'C' 'lmask002_lsem01_semantic_partial_detach' '0.02' '0.10' 'False' 'True' 'True'
            break
        }
        default { throw "Unknown target: $Target. Valid targets: B C" }
    }
}

Run-IndependenceCheck
New-Item -ItemType Directory -Force -Path $ResultsRoot | Out-Null
Set-Content -LiteralPath (Join-Path $ResultsRoot 'stage3_abc_summary.csv') -Value 'tag,exp_name,best_ckpt,selection_md,test_command_template' -Encoding UTF8
foreach ($Target in $Targets) {
    Run-Target $Target
}

Write-Host "Finished stage3 ABC experiments. Summary: $ResultsRoot/stage3_abc_summary.csv"

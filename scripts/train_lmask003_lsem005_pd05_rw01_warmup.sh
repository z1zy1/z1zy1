...
EXP_NAME="lmask003_lsem005_pd05_rw01_warmup"
...
if [ "$OVERWRITE" != "1" ] && [ -d "$EXP_PATH/snapshots" ] && find "$EXP_PATH/snapshots" -type f \( -name '*.pt' -o -name '*.pth' \) | grep -q .; then
  ...
  echo "Use OVERWRITE=1 bash scripts/train_lmask003_lsem005_pd05_rw01_warmup.sh to train again."
  exit 0
fi

echo "========== train $EXP_NAME =========="
echo "lmask=0.003 lsem=0.005 semantic_detach_ratio=0.5 feature_reweight=True reweight_alpha=0.1 aux_warmup=True semantic_decay_final_ratio=0.50"
...
train_cmd=(
  ...
  train.semantic_decay_final_ratio 0.50

...
EXP_NAME="lmask003_lsem005_pd05_warmup_no_reweight"
...
if [ "$OVERWRITE" != "1" ] && [ -d "$EXP_PATH/snapshots" ] && find "$EXP_PATH/snapshots" -type f \( -name '*.pt' -o -name '*.pth' \) | grep -q .; then
  ...
  echo "Use OVERWRITE=1 bash scripts/train_lmask003_lsem005_pd05_warmup_no_reweight.sh to train again."
  exit 0
fi

echo "========== train $EXP_NAME =========="
echo "lmask=0.003 lsem=0.005 semantic_detach_ratio=0.5 feature_reweight=False aux_warmup=True semantic_decay_final_ratio=0.50"
